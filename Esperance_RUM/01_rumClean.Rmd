...
```{r  read data}
# meta <- read_csv("rumIgnore/280222_Meta.csv", na =c("", " ", "na", "NA", "-"))
# ex <- read_csv("rumIgnore/280222_Extractive.csv", na =c("", " ", "na", "NA", "-"))
# nex <- read_csv("rumIgnore/280222_NoNExtractive.csv", na =c("", " ", "na", "NA", "-"))
# avid <- read_csv("rumIgnore/280222_Avidity.csv", na =c("", " ", "na", "NA", "-"))
```
...
esp <- esp %>%
  ...
  mutate(Start = ifelse(str_length(sub(".*:", "", Start)) != 2, str_c(Start, "0"), Start)) %>% 
  # if str_length after colon !=2, add a zero to the end
...
esp <- esp %>%
  ...
  dplyr::filter(!is.na(FieldTrip)) # filtering out submissions out with field trip dates (dummys)
```
...
esp <- esp %>% 
  dplyr::select(
    ID, Date, numYear, facYear, Interviewer, FieldTrip, mLat, mLong, PersonID, Screen18,
         ...
         DiveMaxDepth, WhyChoose, WhyLeave, Detract, exTimes12m, exYrs, nexTimes12m, nexYrs,
         DiveCert, nDives, YrBorn, Postcode, Accom, Sex, nFemales, nMales, nGirls, nBoys, nAdults,
