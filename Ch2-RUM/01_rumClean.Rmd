...
```{r  readData}
meta <- read.csv("rumIgnore/Data-250921/meta_0921.csv", na.strings = c("", " "))
use <- read.csv("rumIgnore/Data-250921/use_0921.csv", na.strings = c("", " "))
avidity <- read.csv("rumIgnore/Data-250921/avidity_0921.csv", na.strings = c("", " "))
...
Ning0921 <- full_join(MetaUse, avidity, by = c("uGlobID" = "aGUID"), keep=T) %>%
  rename(nDate = Date)

Ning <- bind_rows(Ning0921, jon)
```

```{r csv}
write.csv(Ning0921, 'rumIgnore/Data-250921/Ning0921.csv') # My up to date unleaned data 
write.csv(Ning, 'rumIgnore/RAW_Ning.csv') # full raw uncleaned dataset including Jon's data.

rm(avidity, jon, meta, use, MetaUse, Ning0921) # removing temp data sets 
```
...
dplyr::rename(FieldTrip = "Trip.number",
  ...
  dplyr::select(PersonID, nDate, CreationDate.x,jDate, jTime, Interviewer, FieldTrip, BR, jBR, BRLat, BRLong, Site, Screen18,
         ...
         jDaysBoatFishedCat, jBRAvid, nDives, DiveCert, Postcode, YrBorn, Sex, Party, nMales, nFemales, nBoys, nGirls,
...
Ning <- Ning %>%
  ...
  #  check to see whethr you can make this work with the avidity stuff without jankying up the idsbut probably not so will have to redo the ids for avidity after you hav added this line in 
...
Ning <- Ning %>% 
  ...
  mutate(nexnTimes12m = ifelse(ID %in% "832", 365, nexnTimes12m)) %>% 
  mutate(nexYrs = ifelse(ID %in% "832", "Life", nexnTimes12m)) 
...
Ning <- Ning %>%
  ...
  dplyr::mutate(Resident = ifelse(nBoatDays %in% "Resident", "1", "0")) %>% 
  ...
  dplyr::mutate(Resident = ifelse(!is.na(ResnBoatDaysYr), "1", Resident)) %>% 
...
```{r nDaysInArea}
Ning <- Ning %>%
   dplyr::mutate(Resident = ifelse(nDaysInArea_other %in% c("8 weeks - semi resident", "Come up for 4 months of the year and goes out on the boat at least 2 times a week ", "6 months of the year"), "1", Resident)) %>% 
...
## IF you check at unique(Ning2 $ EXTRACTIVE) at this point, row 20-22 have to be attended to at some point - maybe 18, unessecary for now. 
  
  dplyr::mutate(Comments = ifelse(!is.na(EXTRACTIVE), paste(EXTRACTIVE, Comments, sep = "|"), Comments)) 
  # dplyr::select(-EXTRACTIVE)

# a <- which(!is.na(Ning2$EXTRACTIVE))
# Ning2[a, c("EXTRACTIVE", "Comments")]
# b <- which(Ning2$EXTRACTIVE %in% c("Oil rigs", "FADs", "Helby bank to northern FAD, then up and down reef?", "Two rocks", "Oil rigs = location?"))
# Ning2[b, c("UseLat", "UseLong", "FishingType", "MaxHook","EXTRACTIVE", "Site")]
## Fix FADs, oil rigs and two rocks 
## 
## 
## wave bouy fad - navionics, instead all opil rigs, yellow special marker weather marker
## Pipeline onlsow to those rigs 
## two rocks in front of induistrial area 
```
...
Ning <- Ning %>%
  ...
  dplyr::mutate(FishingType = as.factor(FishingType)) %>% 
  dplyr::select(-c(FishingType_other, jFishingType))

# Interropgate casting
# a <- which(Ning$FishingType == "Casting")
# Ning[a, c("MaxHook", "BaitLure")]

# if therey on boat, using biat, demersal , if they on a boat they are using a lure they are casting, shore, based casting. 
```
...
Ning <- Ning %>%
  ...
  dplyr::mutate(BaitLure = as.factor(BaitLure)) %>%
...
dplyr::mutate(Activity_other=ifelse(str_detect(Activity_other,"Swim|swim|play|Relax|castles|Chilling|Drinking|Kid|scenery|Waiting|waiting|Beach|Family|relax|whale|Cruising aroung, skipper ticket "), "Beachgoing", Activity_other)) %>%
  ...
  dplyr::mutate(Activity = ifelse(Activity %in% "JetSki", "Jet skiing", Activity)) %>% 
...
Ning <- Ning%>%
  ...
  relocate(FishLife, FishOcc, .after = nTimesFished) %>% 
  mutate(exYrs = is.numeric(exYrs)) %>% 
...
Ning <- Ning %>% 
  ...
  mutate(Comments = ifelse(exnTimes12m %in% c("Holiday hobby: went fishing yesterday and caught a few fish but bout 6 depredations (said post survey in conversation )", "Don’t fish often only when taken out by other people and never eat fish just do it for fun and release", "3 week  said he only have to fish once a week 10 years ago to get enough fish to eat, now he has to fish 3 times a week to get enough fish to eat. No fish left , when they go out on they boat they get 1 fish out of three taken by sharks and move on but say", "Had been fishing as well but didn’t have enough time "), paste(Comments, exnTimes12m, sep = "|"), Comments)) %>% 
  # Specific things to go into comments
  mutate(Comments = ifelse(exnTimes12m %in% "Ow. A charter so go when chartering", paste(Comments, "Owns a charter so doesnt go out independently as a recreator becuase he doesnt need to, just fishes when he is working", sep = "|"), Comments)) %>% 
   # Cat 1:0-25
  mutate(exnTimes12m = ifelse(exnTimes12m %in% c("Opportunistically-friends boats", "Never, holiday fishers ", "Zero", "Three", "No", "2snhore ", "12 a year ", "No ", "2 a month", "1 in 3 weeks ", "0, used to go a lot, occasionally ", "3, shore 10in 6month", "Shore: 12", "1month", "1 a month", "2month ", "Two", "Just haunt boat ", "1, shore 4", "1a month boat ", "Maybe 5 dunno hard to remember ", "O", "2amonth half shore half boat. Have a boat in Perth ", "3yesr, shore ", "Boat: 2 month", "Shore: month 1", "1month shorev", "Occasionally when friends offer, every month or so", "2 month", "0 1sr time learning", "Ocassional spearing", "Monthly", "Don’t fish often only when taken out by other people and never eat fish just do it for fun and release", "Year;y", "Shore every week when had boat but now maybe once a month", "Holiday hobby", "10  ho;ODST hobby", "20 plus", "1 month", "1 monthboat", "1/month", "Couple of months", "Twice yearly", "2 mo th", "Every couple of months ", "10 hrs monthly", "Month 2 ", "Holiday hobby: went fishing yesterday and caught a few fish but bout 6 depredations (said post survey in conversation )", "1 m0nth", "1 month 5 year", "2mo th", " One", "2month", "5 year", "1mo th", "Holiday gphobby", "15, everyday in school ho,idays "), "1", exnTimes12m)) %>% 
  # Cat 2: 26-50
  mutate(exnTimes12m = ifelse(exnTimes12m %in% c("Not sure; a couple of times a month as a family, my husband goes a lot more.", "1 fortnite", "5 weeks a year ", "5 weeks ", "50 estimate", "2a month, in such holidays 1 2 week ", "Fortnightly", "4 weeks on boat    ", "3 times per month, squidding", "Fortnight", "3 month", "1formoght", "Month", "2 month, 2weeks every day", "Once a fortnight ", "Fortmightly", "Dunno we go a bit but not weekly"), "30", exnTimes12m)) %>% 
  # Cat 3: 51-75
  mutate(exnTimes12m = ifelse(exnTimes12m %in% c("Once a week", "3/4 @ month ", "3/4 a month ", "Weekly", "1a week ", "Every weekend", "4 in month sore", "1 week", "1 week - wish I could do more but work always gets in the way", "4 month", "1weekly", "1week"), "60", exnTimes12m)) %>% 
  # Cat 4 : 76-100
  mutate(exnTimes12m = ifelse(exnTimes12m %in% c("Month 7", "3 months worth", "60 -100"), "80", exnTimes12m)) %>% 
  # Cat 5: 100+
  mutate(exnTimes12m = ifelse(exnTimes12m %in% c("Too many to count", "250 shore based ", "Shore10 months ", "20a month ", "Everyday ", "365 shore", "3 weekly", "12 days month", "Every second day when Crabs", "Ow. A charter so go when chartering", "5week", "9/month", "5 month", "26 month", "Daily", "4 times a week", "2 week ", "6 month", "7 month", "Twice a week", "2 a week ", "2 weekly"), "150", exnTimes12m)) %>% 
    mutate(exnTimes12m = ifelse(exnTimes12m %in% c("3 week  said he only have to fish once a week 10 years ago to get enough fish to eat, now he has to fish 3 times a week to get enough fish to eat. No fish left , when they go out on they boat they get 1 fish out of three taken by sharks and move on but say", "Had been fishing as well but didn’t have enough time ", "Shore only fish in eddy 2/3 time while on trip"), NA, exnTimes12m)) %>% 
  mutate(exnTimes12m = as.numeric(exnTimes12m)) %>% 
  mutate(exnTimes12m = ifelse(between(exnTimes12m, 0, 25), 1, exnTimes12m)) %>% 
  mutate(exnTimes12m = ifelse(between(exnTimes12m, 26, 50), 2, exnTimes12m)) %>% 
  mutate(exnTimes12m = ifelse(between(exnTimes12m,51, 75), 3, exnTimes12m)) %>% 
  mutate(exnTimes12m = ifelse(between(exnTimes12m, 76, 100), 4, exnTimes12m)) %>% 
  mutate(exnTimes12m = ifelse(exnTimes12m > 100, 5, exnTimes12m)) %>%
  mutate(exnTimes12m = ifelse(numYear < "2020", jDaysBoatFishedCat, exnTimes12m)) %>% 
  dplyr::select(-jDaysBoatFishedCat)
```
...
Ning <- Ning %>% 
  ...
  mutate(Resident = ifelse(Postcode %in% 6707, 1, Resident))
