...
## Testing Tab with To do list 
Next time you have to -> 

*Make a section on housekeeping rules, this should include naming conventions, links to where I can get help, file structure and all the rest of it.

•Test how you source functions and library (if you put them all on the mother, does that mean that you cant run them in your child docs?)

•test how the Rmd file upload to GItHub and consider how ypu will share and collaborate with Tim and Matt.

•Write a ReadMe with housekeeping rules

*can you cross reference bwteen Rmd docs??

*Once your happy with it you will have to share this with Tim PhD student folder

# Merging Meta 
After downloading the data from arcCollector, you end up thre individual .csv for each survey layer, meta.csv, use.csv, avidity.csv. Merge the three sets of data, linking them by their GlobalID and GUID. This is a two step process;

1. Merge meta.csv and Use.csv to make MetaUse(ddyy).csv eg. MetaUse0920.csv for the fieldtrip that began in 09/20
...
```{r irisPlot, fig.cap= "Iris Matrix"}
plot(iris)
```

```{r irisModel}

lm(Species~Petal.Length, data=iris)

```

see figure \@ref(fig:irisPlot)
