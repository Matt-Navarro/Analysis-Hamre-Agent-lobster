...
#### Filter ####
# The following section is separating out Harrisons 2018 data so I can remove it from data set. 
  
  mutate(nDate = ifelse(
    nDate %in% "", CreationDate.x, nDate)) %>% 
  # Fill Date col NAs with values from CreationDate.x: mutate(newcol = ifelse(condition, True, False))
  select(-c(CreationDate.x)) %>% # Removing CreationDate.x col
  separate(nDate, into = c("Date", "Time", "ampm"), sep = " ", remove = TRUE) %>% # separating date-time var
  separate(Date, c("dm", "md", "Year"), sep = "/", remove = FALSE) %>% #Separating dtae to get Year col
  filter(is.na(Year) | Year != 2018) %>% # Removing observation from 2018. is.na(Year) retains NAs for Jons data
  select(-c(dm, md)) %>% # removing excess cols
  filter_all(all_vars(!grepl('ummy', .))) %>% # getting rid of Dummys/dummys
  filter_all(all_vars(!grepl("lah", .)))  # getting rid of blah blahs
  
  slice(-(1:4)) # empty observations -> find way to automate and row
...
### Date and Time
Currently variables associated with date and time are: Date, Year, Time, ampm, jDate, jTime
...
names(jon)
```
After doing a explore of the columns here is the state of the current date/time associated cols and changes that need to be made. 

| Column | Current state | Changes to be made |
|:------:|:-------------:|:------------------:|
|Date|For 2021/21, mdY, character| Need to add in Jons dates, change to as.date|
|Year|For 2021/21, NAs are where Jons years need to go, character|Append Jons years|
|Time|For 2020/21, no surveys were doen before 9 checked metadata time is -8h (uk tz), character| Need to add 8h and get into 24h|
|ampm|Contains am and pm associated with time | will be redundant when time is in 24h|
|jDate| Jons Dates, dmY, character|Need to make same format as Date, append and remove|
|jTime|Jon time, assume correct in 24h| Append to time|

```{r DateClean}
Ning <- Ning %>%
  separate(Date, into = c("m", "d", "Y"), sep = "/", remove = TRUE) %>%
  unite("Date", d, m, Y, sep="/") %>%
  mutate(Date = ifelse(Date %in% "NA/NA/NA", jDate, Date)) %>%
  select(-c(jDate))

names(Ning)  
unique(Ning$Date)
Ning[1:4,] # check values of rows

```

to do list 
* See how to can filter out all rows which contain only NA values
* See if you can get your Date col to as.date format


```{r}

## useful code ill need at some point
 unique(Ning$EXTRACTIVE)
 unique(Ning$NONEXTRACTIVE)
 unique(Ning$nDaysInArea_other)
 unique(Ning$nBoatDays_other)
 unique(Ning$nShoreDays_other)
 unique(Ning$nTimesLast12m_other)
 unique(Ning$nTimes19_other)
 unique(Ning$FishingType_other)
 unique(Ning$KeptUdam_other)
 unique(Ning$RelUndam_other)
 unique(Ning$nDP_other)
 unique(Ning$exWhyLeave_other)
 unique(Ning$Activity_other)
 unique(Ning$WhyChoose_other)
 unique(Ning$nexWhyLeave_other)
 unique(Ning$Accom_other)
 unique(Ning$FINISH.SURVEY)
 unique(Ning$Party)
 
ifelse(str_detect(df$col, "words to change|bjsbbdsjdb|jbsdjsdb"), "new name",
       ifelse())
       
which(colnames(Ning)=="jTime") # lets you find col number for specified col
