...
# The above variables have been deselected because they are unnecessary, ID vars will be replaced with a new ID col, Creator and Editor is always Harrison, only date variable I need is date which was inputted by interviewer and CreationDate.x incase it has been missed
  ...
  # Now that my tbl is a little less unwieldy I am going to start cleaning starting from the leftmost column and work along the data set. 
...
## work up to here, to do for next time: Date column cannot be changed from char -> date because it has multiple date formats. Mine and Harry's is formatted as mdY, and Jons is formatted as dmY. Need to either parse and reformat the current date column or format date before uniting them.
  ...
  parse_date_time(Date(), tz = "Australia/Perth", c("%m/%d/%Y", "%d/%m/%Y"), exact = TRUE) %>%
