...
# format: rename(newname = currentname)
  ...
  mutate(Date = ifelse(Date %in% "", CreationDate.x, Date)) %>% #Fill Date col NAs with values from CreationDate.x 
  select(-c(CreationDate.x)) %>% # Removing creation date col
  separate(Date, into = c("Date", "Time", "ampm"), sep = " ", remove = TRUE) %>%
  separate(Date, c("dm", "md", "Year"), sep = "/", remove = FALSE) %>%
  filter(Year != 2018) %>%
  select(-c(dm, md)) %>%
  filter_all(all_vars(!grepl('ummy', .))) %>% # getting rid of Dummys/dummys
  filter_all(all_vars(!grepl("lah", .)))  # getting rid of blah blahs 

  # Df is no in a manageable format and a little less unwieldy I am going to start cleaning starting from the    leftmost column and work along the data set. 
  
names(Ning)
unique(Ning$Date)
unique(Ning$Interviewer)
names(Ning0421)
...
```
  ...
  * Date column is character with two formats, all dates in 2020/21 are formatted at mdY, while 2015/16 dmY. Need to get all dates in same format and make column as.date
  
  * Time, Times in 2021/20 are UK time and need have 8 h added onto them. Then PM values need to be made inti the 24h clock
  
  * Interviewer, jon needs to be added for 2015/16 studies. Blanks in 20/21 need to be figured out 
...
## work up to here, to do for next time: Date column cannot be changed from char -> date because it has multiple date formats. Mine and Harry's is formatted as mdY, and Jons is formatted as dmY. Need to either parse and reformat the current date column or format date before uniting them.
