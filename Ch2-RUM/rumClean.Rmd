...
Ning <- Ning %>%
  ...
  dplyr::mutate(nexStop = ifelse(str_detect(nexStop, "[S, s]till|Hasn’t ended"), "StillThere", nexStop)) %>%
...
# Need to finish this section
  # mutate(nexStop = ifelse(str_detect(nexStop, "houts"), "16:00", nexStop))%>% 
  #   mutate(nexStop = ifelse(str_detect(nexStop, "hour"), "14:00", nexStop))%>%
  #   mutate(nexStop = ifelse(str_detect(nexStop, "minss"), "15:00", nexStop))%>% 
  #   mutate(nexStop = ifelse(str_detect(nexStop, "45m"), "08:45", nexStop))%>%
  #   mutate(nexStop = ifelse(str_detect(nexStop, "Now"), "16:50", nexStop))%>%
  #   mutate(nexStop = ifelse(str_detect(nexStop, "Qtep"), "15:30", nexStop))%>%
  #   mutate(nexStop=ifelse(PersonID == 385, "1500", nexStop)) %>%
  ...
  #   mutate(nexStop=ifelse(is.na(nexStop), jnexStop, nexStop))%>%
  #   mutate(nexStop=ifelse(str_length(nexStop)==4, str_c("0", nexStop), nexStop))%>%
  #   relocate(StillThere, .after = nnexStop) %>%
  #   dplyr::select(-TimeOfDay)%>%
  #   dplyr::select(-jnexStop)
```
...
Ning <- Ning %>%
  ...
  dplyr::mutate(BoatID = ifelse(str_detect(BoatID, "[H, h]ire|[R,r]ental"), "Rental", BoatID)) %>%
  ...
  mutate(BoatName = ifelse(BoatID %in% c("Guided missile","Youngbloods", "Jenny", "Fatal attraction", "Lindy Lou", "Sharkcat", "BRIG", "Free radical", "Finley’s", "Cut snake"), BoatID, BoatName)) %>%
  mutate(BoatID = ifelse(BoatID %in% c("Guided missile","Youngbloods", "Jenny", "Fatal attraction", "Lindy Lou", "Sharkcat", "BRIG", "Free radical", "Finley’s", "S201tcm trailer licence plate, no boat number", "Unknown", "Unknown, car licence plate 1acg 426", "Cut snake"), NA, BoatID)) %>%

  dplyr::mutate(BoatID = gsub("[ ]", "", BoatID)) %>%
  dplyr::mutate(BoatID= toupper(BoatID)) %>%
  dplyr::relocate(BoatName, .after= BoatID) 

#### Mutating PersonID so that people with the same bota ID have the same associated PersonID
...
Ning <- Ning %>%
  ...
  dplyr::mutate(FishingType = ifelse(FishingType_other %in% c("Lining ","Casting"), "Casting", FishingType)) %>%
  dplyr::mutate(FishingType = ifelse(FishingType_other %in% "Prawning", "Prawning", FishingType)) %>%
  dplyr::mutate(FishingType = ifelse(FishingType_other %in% c("Drift","Bottom bashin","Drifting"), "Demersal", FishingType)) %>%
  dplyr::mutate(FishingType = ifelse(FishingType_other %in% c("Marlin trawling","Sports and game fishing"), "Trolling", FishingType)) %>%
...
Ning <- Ning %>%
  ...
  dplyr::mutate(KeptUndam_other = ifelse(KeptUndam_other %in% c("1 spangled emperor; 1 threadfin ",  "2 squid"), "2", KeptUndam_other)) %>%
...
Ning <- Ning %>%
  ...
  dplyr::mutate(CaughtUndam = ifelse(numYear > 2016 & is.na(CaughtUndam) & ActivityType %in% "Both"|Activity == "Fishing", "0", CaughtUndam)) %>%
...
Ning <- Ning %>%
  ...
  dplyr::mutate(exWhyLeave_other = ifelse(str_detect(exWhyLeave_other, "luck"), "LowCatch", exWhyLeave_other)) %>%
  ...
  dplyr::mutate(Comments = ifelse(exWhyLeave_other %in% c("Saw a shark", "Note they also had a jet ski- jet ski and dinghy were together", "Was trolling to next site ", "Started catching sharks", "Caught shark", "What to go round to ex marjona and freestyle JetSki"), paste(exWhyLeave_other, Comments, sep = "|"), Comments)) %>%
  
  dplyr::mutate(exWhyLeave_other = ifelse(str_detect(exWhyLeave_other, "Caught shark|Depredation |Sharks|Saw a shark|Started catching sharks"), "Depredation", exWhyLeave_other)) %>%
  
  dplyr::mutate(exWhyLeave_other = ifelse(exWhyLeave_other %in% c("Haven’t left yet", "Still going", "While I interviewed them they were still there so I didn’t ask but I saw them leave at 1020", "Still fishing", "Did both bottom trawling and bottom bashing; interviewed about bottom bashing", "Was trolling to next site ", "Note they also had a jet ski- jet ski and dinghy were together", "What to go round to ex marjona and freestyle JetSki"), NA, exWhyLeave_other)) %>%
  
  dplyr::mutate(exWhyLeave = ifelse(!is.na(exWhyLeave_other), exWhyLeave_other, exWhyLeave)) %>%
...
#  st_write(StudySite, paste(s.dir, "StudySite.gpkg", sep = '/'))

# Cropping coast to study site
# NWS <- st_crop(WA, StudySite)
# NWS <- st_union(NWS)
# st_write(NWS, paste(s.dir, "NWScoast.gpkg", sep = '/'))
...
HabitatMap <- ggplot() +
  ...
  scale_fill_manual(values=c("aquamarine", "light blue", "coral", "pink"))
...
spNing <- spNing %>%
  ...
  ungroup() %>%
  relocate(GridID, DistNearestBRkm, DistNearestWellkm, DistNearestPipekm, DistArtReefkm, DistNearestInfra, Depth, Habitat, LunarPhase, UseLat, UseLong, .before = geometry) 
```

```{r save as shape file}
st_write(spNing, paste(s.dir, "spNing.gpkg", sep = '/'), append = FALSE)
```
...
write.csv(spNing, paste(d.dir, "Ning_v2.csv", sep = "/"))
```
...
spNing <- spNing %>%
  dplyr::mutate(SiteType = ifelse(ID %in% c("1044", "1092", "471", "1225", "450", "955", "646", "678", "980", "788", "643", "538", "542", "524", "677", "1261",  "1233", "754", "800", "836", "760", "755", "923", "871", "985", "673", "1296", "876", "878", "730", "725", "719", "1108", "1231", "1232", "798", "799", "758", "1254"), "Shore", SiteType)) %>%
...
ggplot() + 
  ...
  geom_sf(data = spNing, size = 0.05, aes(colour=SiteType))
```
The next variables will be done for each subset of data:
All activities (spNing)

Fishing (f)
Boat based fishing (bf)
Shore based fishing (sf)

Non-extractive activities (ne)
Boat based non-extractive activities (bne)
Shore based non-extractive activities (sne)

```{r 5km}
spNing_bf <- spNing %>%
  filter(SiteType %in% "Boat", ActivityType %in% c("Extractive", "Both"))  # filtering by boat fishing

pointBuffer_bf = st_buffer(spNing_bf, 5000)

#  Checking buffer
# ggplot() +
#   geom_sf(data = NWS) +
#   geom_sf(data = pointBuffer_bf) +
#   geom_sf(data = spNing, size = 0.05)

intersect_bf = st_intersects(spNing_bf, pointBuffer_bf) # how to filter this by date?

bf5km = lengths(intersect_bf)

# This would be final output for just a fishing data set - would it be better to ahve lots of separate ones or one big one?
spNing_bf <- spNing_bf %>%
  mutate(bf5km = bf5km)
  
# if you are going to have one big one how do you append it?
spNing2 <- spNing %>%
  mutate(bf5km = ifelse(spNing$ID == spNing_bf$ID, spNing_bf$bf5km, NA))
  
  st_join(spNing, spNing_bf, )
```

```{r Kernal Density}

## Using denisty from spatstat
spNing_Lam <- st_transform(spNing, crs = 3112)
...
Ningraster <- spNingkm_Lam$KernalDensity
Ningraster <- rescale(Ningraster, 1/1000) #Need to rescale so the coords match up with the original ones from the dataset
Ningraster <- raster(Ningraster) #Conver to raster

Points <- st_as_sf(spNing$geometry) #Convert your dataframe points in a simple shape file

#Check they plot over the top of each other and therefore that the coordinates are the same
plot(Ningraster)
plot(Points, add=T) #If you want to plot this on top you need to run this line and the above line in the consol as RMarkdown won't let you plot one on top of the other 

# Extract the value in the raster where your points are 
KDEPoints <- raster::extract(Ningraster, Points, fun=max)
