...
Ning <- Ning %>%
  mutate(TimeOfDay = ifelse(str_detect(nexStop, "pm"), "pm",
   ...
   mutate(nexStop = ifelse(str_detect(nexStop, "[S, s]till|Hasn’t ended"), "StillThere", nexStop)) %>%
...
c <- which(is.na(Ning$SiteType) & !is.na(Ning$UseLat) & Ning$Agreement %in% "Yes")

N6[c, ]

a <- which(is.na(Ning$UseLat) & Ning$Agreement == "Yes")
Ning[a,]

c <- N6 %>%
  filter(is.na(SiteType) & !is.na(N6$UseLat) & N6$Agreement %in% "Yes" & N6$ActivityType %in% c("Extractive", "Both"))
...
ggplot() +
  ...
  geom_sf(data = NWS) +
  geom_sf(data = spNing1, size = 0.05, aes(colour=SiteType)) 
...
```{r spSiteType}
spNing1 <- spNing %>%
  filter(is.na(SiteType) & Agreement %in% "Yes")
  mutate(SiteType2 = ifelse(st_intersects(spNing, NWS), "Shore", SiteType)) %>%
    
    
  mutate(SiteType = ifelse(is.na(SiteType) & Agreement %in% "Yes", SiteType2, SiteType))


spNing1[c, c("SiteType", "SiteType2")]
which(is.na(spNing1$SiteType) & spNing1$Agreement == "Yes")


```

```{r 5km}
spNing1 <- st_buffer(spNing, )
...
st_write(Ning, paste(s.dir, "spNing.gpkg", sep = '/'), append = FALSE)
```
...
write.csv(Ning, paste(d.dir, "Ning_v2.csv", sep = "/"))
```
