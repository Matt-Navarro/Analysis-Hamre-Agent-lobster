...
#### Fishing Type ####
  mutate(FishingType_other = ifelse(str_detect(FishingType_other, "we’re just squidding|Going for squid|Squidding"), "Squidding", FishingType_other),
         ...
         FishingType = ifelse(Year == "2015", jFishingType, FishingType),
         FishingType = ifelse(Year == "2016", jFishingType, FishingType)) %>%
#### exStart ####
...
# Standardise and append other
 mutate(RelUndam_other = ifelse(str_detect(RelUndam_other, "Rel|Too"), "1", RelUndam_other),
        ...
        RelUndam = ifelse(KeptUndam > 0 & is.na(RelUndam), "0", RelUndam),
        RelUndam = ifelse(KeptUndam == "0" & is.na(RelUndam), "0", RelUndam),
...
# Append Rel Undam + Kept Undam to 2020/21 values
  mutate(jCaughtUndam = ifelse(Year > "2016", RelUndam + KeptUndam, jCaughtUndam)) %>%
...
# if Year = 2020/21, then = nDP/nHooked
 mutate(perDP = ifelse(Year == "2020", nDP/nHooked*100, perDP),
        perDP = ifelse(Year == "2021", nDP/nHooked*100, perDP)) %>%
  relocate(CaughtUndam, nHooked, perDP, proDP, binDP, .before = Species) %>%
