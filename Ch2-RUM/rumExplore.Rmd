...
```{r ActTyp}
 ActTyp <- ggplot(N) +
   ...
   labs(x = "Activity Type", y = paste("Frequency (n = ", nrow(N), ")", sep = "")) +
...
ActFreq <- N %>% 
  ...
  labs(x = paste("Activity (n = ", length(na.omit(unique(N$Activity))), ")", sep=""), y = paste("Frequency (n = ", nrow(N), ")", sep = "")) +
...
write.csv(dpDat, paste(d.dir, "dpDat_v1.csv", sep = "/"), row.names = FALSE)
```
